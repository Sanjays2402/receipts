// Receipts — Costco US (costco.com) extractor.
//
// Pulls order id, date, USD total, and line items off Costco's
// order-details / order-status pages:
//   https://www.costco.com/OrderStatusCmd
//   https://www.costco.com/OrderStatusDetailsView?...
//   https://www.costco.com/my-account/orders/<orderId>
//
...
const MERCHANT_ID = "costco";
const CURRENCY = "USD";

/** True iff this URL is a Costco.com orders page. */
export function matchesCostco(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  if (u.hostname !== "www.costco.com" && u.hostname !== "costco.com") return false;
  const p = u.pathname || "";
  return /\/(OrderStatus|OrderDetails|my-account\/orders|orders)/i.test(p);
}
...
/** Parse "$1,234.50" / "USD 99" into a Number. */
...
/** Parse Costco date strings like "May 23, 2026" or "Ordered May 23, 2026". */
export function parseCostcoDate(str) {
  if (!str) return null;
...
/** Extract the order id. Costco uses long numeric/alphanumeric ids. */
export function extractOrderId(doc, url) {
  if (url && typeof url === "string") {
    try {
      const u = new URL(url);
      const m = u.pathname.match(/\/(?:my-account\/orders|orders|OrderDetails|OrderStatusDetailsView)\/([A-Z0-9-]{4,})/i);
      if (m && m[1]) return m[1];
      const qp = u.searchParams.get("orderNumber") || u.searchParams.get("orderId");
      if (qp) return qp;
    } catch { /* ignore */ }
  }
...
export function extractItems(doc) {
  ...
  for (const row of rows) {
    const nameEl =
      ...
      row.querySelector?.(".productTitle") ||
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseCostcoDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
...
const costcoExtractor = {
  id: MERCHANT_ID,
  version: "0.1.0",
  matches: (url) => matchesCostco(url),
  extract,
};
