// Receipts — Snapdeal (snapdeal.com) extractor.
//
// Pulls order id, date, INR total, and line items from Snapdeal's
// order pages:
//   https://www.snapdeal.com/orders
//   https://www.snapdeal.com/orders/<orderId>
//
// Snapdeal renders server-side-ish HTML with stable class hints
// ("order-card", "order-summary") plus data-test hooks, so we try
// data-* attributes first and fall back to visible-text patterns
// ("Order ID", "Grand Total", "Total Paid", the rupee glyph).
//
// Tests live in tests/extractors/snapdeal.test.mjs.

const MERCHANT_ID = "snapdeal";
const CURRENCY = "INR";

/** True iff this URL is a Snapdeal order page. */
export function matchesSnapdeal(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  const host = u.hostname;
  if (host !== "snapdeal.com" && host !== "www.snapdeal.com") return false;
  const p = u.pathname || "";
  return (
    p === "/orders" ||
    p.startsWith("/orders/") ||
...
/** Parse Snapdeal date strings: "12 May 2026", "12-May-2026", ISO. */
export function parseSnapdealDate(str) {
  if (!str) return null;
  const WEEKDAYS = /^(?:mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)[a-z]*\.?,?\s*/i;
  const cleaned = String(str)
    .replace(/^.*?(?:delivered on|delivered at|ordered on|order placed|placed on|purchased on|on)\s*/i, "")
    .replace(WEEKDAYS, "")
    .replace(/(\d+)(st|nd|rd|th)/gi, "$1")
    .replace(/[,]/g, " ")
    .trim();

  // "12 May 2026" / "12 May '26" / "12-May-2026"
...
/**
 * Extract Snapdeal's order id. Snapdeal exposes ids as long numeric
 * strings ("123456789012") or "SD-<digits>" slugs, via URL
 * (/orders/<id>) and as "Order ID: <id>" in the DOM.
 */
export function extractOrderId(doc, url) {
  if (url && typeof url === "string") {
    try {
      ...
      if (idx >= 0 && parts.length > idx + 1) {
        ...
        if (/^(?:SD-?)?[A-Z0-9]{6,32}$/i.test(cand)) return cand;
      }
    } catch { /* ignore */ }
  }
  ...
  let m = txt && txt.match(/(?:SD-?)?[A-Z0-9]{6,32}/i);
  if (m) return m[0];
  const all = nodeText(doc.querySelector?.("body") || doc);
  m = all && all.match(/Order\s*(?:ID|No\.?|Number)[:\s]*((?:SD-?)?[A-Z0-9]{6,32})/i);
  if (m) return m[1];
  return null;
}

/** Pull line items from Snapdeal order rows. */
...
export function extractTotal(doc) {
  ...
  // Fallback: summary rows labelled "Grand Total" / "Total Paid" / "You Paid".
  const rows = doc.querySelectorAll?.(
  ...
  if (rows) {
    for (const row of rows) {
      ...
      if (!text) continue;
      if (/grand total|order total|bill total|amount paid|amount payable|total payable|net payable|you paid|total amount|total paid/.test(text)) {
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseSnapdealDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
...
const snapdealExtractor = {
  id: MERCHANT_ID,
  version: "0.1.0",
  matches: (url) => matchesSnapdeal(url),
  extract,
};

export default snapdealExtractor;
