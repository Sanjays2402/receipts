// Receipts — BigBasket (bigbasket.com) extractor.
//
// Pulls order id, date, INR total, and line items from BigBasket's
// account order pages:
//   https://www.bigbasket.com/member/order-history/
//   https://www.bigbasket.com/member/order-details/<orderId>/
//   https://www.bigbasket.com/order/<orderId>/
//
// BigBasket's DOM ships hashed CSS-module class names, so we lean on
// stable data-* attributes when present and fall back to visible-text
// patterns ("Order ID", "Order Total", "Amount Paid", the rupee glyph).
//
// Tests live in tests/extractors/bigbasket.test.mjs.

const MERCHANT_ID = "bigbasket";
const CURRENCY = "INR";

/** True iff this URL is a BigBasket order page. */
export function matchesBigBasket(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  if (u.hostname !== "www.bigbasket.com" && u.hostname !== "bigbasket.com") return false;
  const p = u.pathname || "";
  return (
    p.startsWith("/member/order-history") ||
    p.startsWith("/member/order-details") ||
    p.startsWith("/member/orders") ||
    p.startsWith("/order/")
  );
}
...
/** Parse BigBasket date strings: "Placed on 23 May 2026", "23-May-2026", ISO. */
export function parseBigBasketDate(str) {
  if (!str) return null;
...
/**
 * Extract BigBasket's order id. BigBasket exposes ids like
 * "BB-12345678" or 8–18 digit numeric strings, via URL
 * (/member/order-details/<id>/, /order/<id>/) and as "Order ID: <id>"
 * in the DOM.
 */
export function extractOrderId(doc, url) {
  if (url && typeof url === "string") {
    try {
      ...
      let idx = parts.indexOf("order-details");
      if (idx < 0) idx = parts.indexOf("order");
      if (idx < 0) idx = parts.indexOf("orders");
      if (idx >= 0 && parts.length > idx + 1) {
        const cand = parts[idx + 1];
        if (/^[A-Z0-9\-]{6,24}$/i.test(cand)) return cand;
      }
    } catch { /* ignore */ }
  }
  ...
  let m = txt && txt.match(/[A-Z0-9\-]{6,24}/i);
  if (m) return m[0];
  const all = nodeText(doc.querySelector?.("body") || doc);
  m = all && all.match(/Order\s*(?:ID|No\.?|Number)[:\s]*([A-Z0-9\-]{6,24})/i);
  if (m) return m[1];
  return null;
}

/** Pull line items from BigBasket order rows. */
...
export function extractTotal(doc) {
  ...
  // Fallback: summary rows labelled "Order Total" / "Amount Paid" / "Grand Total".
  const rows = doc.querySelectorAll?.(
  ...
  if (rows) {
    for (const row of rows) {
      ...
      if (!text) continue;
      if (/grand total|order total|amount paid|amount payable|total payable|net payable|you paid|total amount/.test(text)) {
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseBigBasketDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
...
const bigbasketExtractor = {
  id: MERCHANT_ID,
  version: "0.1.0",
  matches: (url) => matchesBigBasket(url),
  extract,
};

export default bigbasketExtractor;
