// Receipts — Walmart US (walmart.com) extractor.
//
// Pulls order id, date, USD total, and line items off the canonical
// order-details page:
//   https://www.walmart.com/orders/<orderId>
//
...
const MERCHANT_ID = "walmart";
const CURRENCY = "USD";

/** True iff this URL is a Walmart.com orders page. */
export function matchesWalmart(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  if (u.hostname !== "www.walmart.com" && u.hostname !== "walmart.com") return false;
  const p = u.pathname || "";
...
/** Parse Walmart date strings like "May 23, 2026" or "Ordered May 23, 2026". */
export function parseWalmartDate(str) {
  if (!str) return null;
  const cleaned = String(str)
    .replace(/^.*?(?:ordered on|ordered|order placed|placed on|order date:?|delivered on|delivery date:?)\s*/i, "")
    .trim();
...
/** Extract the order id. Walmart uses long numeric ids in path /orders/<id>. */
export function extractOrderId(doc, url) {
  if (url && typeof url === "string") {
    try {
      const u = new URL(url);
      const m = u.pathname.match(/\/orders\/([A-Z0-9-]+)/i);
      if (m) return m[1];
    } catch { /* ignore */ }
  }
...
export function extractTotal(doc) {
  ...
  const rows = doc.querySelectorAll?.(
    "[data-testid='order-summary'] div, .order-summary div, .order-summary tr, tr",
  );
  if (rows) {
    for (const row of rows) {
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseWalmartDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
...
const walmartExtractor = {
  id: MERCHANT_ID,
  version: "0.1.0",
  matches: (url) => matchesWalmart(url),
  extract,
};
