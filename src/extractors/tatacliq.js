// Receipts — Tata CLiQ (tatacliq.com) extractor.
//
// Pulls order id, date, INR total, and line items from Tata CLiQ's
// account order pages:
//   https://www.tatacliq.com/my-account/orders
//   https://www.tatacliq.com/my-account/orders/<orderId>
//   https://www.tatacliq.com/my-account/order-details/<orderId>
//
// Tata CLiQ's DOM uses hashed/CSS-module class names, so we lean on
...
//
// Tests live in tests/extractors/tatacliq.test.mjs.

const MERCHANT_ID = "tatacliq";
const CURRENCY = "INR";

/** True iff this URL is a Tata CLiQ order page. */
export function matchesTataCliq(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  if (u.hostname !== "www.tatacliq.com" && u.hostname !== "tatacliq.com") return false;
  const p = u.pathname || "";
  return (
    p.startsWith("/my-account/orders") ||
    p.startsWith("/my-account/order-details") ||
    p.startsWith("/myaccount/orders")
  );
}
...
/** Parse Tata CLiQ date strings: "Placed on 23 May 2026", "23-May-2026", ISO. */
export function parseTataCliqDate(str) {
  if (!str) return null;
...
/**
 * Extract Tata CLiQ's order id. Tata CLiQ exposes ids like
 * "TC2026052312345" or 10–22 alphanumeric strings, via URL
 * (/my-account/orders/<id>, /my-account/order-details/<id>) and as
 * "Order ID: <id>" in the DOM.
 */
...
/** Pull line items from Tata CLiQ order rows. */
...
export function extractTotal(doc) {
  ...
  if (rows) {
    for (const row of rows) {
      ...
      if (!text) continue;
      if (/grand total|order total|amount paid|amount payable|total payable|net payable|you paid/.test(text)) {
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseTataCliqDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
...
const tatacliqExtractor = {
  id: MERCHANT_ID,
  version: "0.1.0",
  matches: (url) => matchesTataCliq(url),
  extract,
};

export default tatacliqExtractor;
