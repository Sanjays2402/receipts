// Receipts — Target US (target.com) extractor.
//
// Pulls order id, date, USD total, and line items off the canonical
// order-details page:
//   https://www.target.com/orders/<orderId>
//   https://www.target.com/account/orders/<orderId>
//
...
const MERCHANT_ID = "target";
const CURRENCY = "USD";

/** True iff this URL is a Target.com orders page. */
export function matchesTarget(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  if (u.hostname !== "www.target.com" && u.hostname !== "target.com") return false;
  const p = u.pathname || "";
  return p.startsWith("/orders") || p.startsWith("/account/orders");
}
...
/** Parse Target date strings like "May 23, 2026" or "Placed May 23, 2026". */
export function parseTargetDate(str) {
  if (!str) return null;
...
/** Extract the order id. Target uses long alphanumeric ids in path /orders/<id>. */
...
export function extractTotal(doc) {
  ...
  const rows = doc.querySelectorAll?.(
    "[data-test='order-summary'] div, [data-testid='order-summary'] div, .order-summary div, .order-summary tr, tr",
  );
  if (rows) {
    for (const row of rows) {
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseTargetDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
