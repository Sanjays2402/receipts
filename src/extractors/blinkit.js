// Receipts — Blinkit (blinkit.com) extractor.
//
// Pulls order id, date, INR total, and line items from Blinkit's
// account order pages:
//   https://blinkit.com/account/orders
//   https://blinkit.com/account/orders/<orderId>
//   https://blinkit.com/order/<orderId>
//
// Blinkit ships hashed CSS-module class names, so we lean on stable
// data-* attributes when present and fall back to visible-text patterns
// ("Order ID", "Bill Total", "Grand Total", the rupee glyph).
//
// Tests live in tests/extractors/blinkit.test.mjs.

const MERCHANT_ID = "blinkit";
const CURRENCY = "INR";

/** True iff this URL is a Blinkit order page. */
export function matchesBlinkit(url) {
  if (!url || typeof url !== "string") return false;
  let u;
  try { u = new URL(url); } catch { return false; }
  if (u.hostname !== "blinkit.com" && u.hostname !== "www.blinkit.com") return false;
  const p = u.pathname || "";
  return (
    p.startsWith("/account/orders") ||
    p.startsWith("/account/order") ||
    p.startsWith("/order/") ||
    p.startsWith("/orders/")
  );
}
...
/** Parse Blinkit date strings: "23 May 2026, 04:12 PM", "23-May-2026", ISO. */
export function parseBlinkitDate(str) {
  if (!str) return null;
...
/**
 * Extract Blinkit's order id. Blinkit exposes ids like
 * "GR-12345678" or 10–20 digit numeric strings, via URL
 * (/account/orders/<id>, /order/<id>) and as "Order ID: <id>" in DOM.
 */
...
/** Pull line items from Blinkit order rows. */
...
export function extractTotal(doc) {
  ...
  // Fallback: summary rows labelled "Bill Total" / "Grand Total" / "Amount Paid".
  const rows = doc.querySelectorAll?.(
...
export function extractDate(doc) {
  ...
  for (const sel of candidates) {
    const iso = parseBlinkitDate(nodeText(doc.querySelector?.(sel)));
    if (iso) return iso;
  }
  return null;
}
...
const blinkitExtractor = {
  id: MERCHANT_ID,
  version: "0.1.0",
  matches: (url) => matchesBlinkit(url),
  extract,
};

export default blinkitExtractor;
