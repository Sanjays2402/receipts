// Tests for src/extractors/myntra.js
import assert from "node:assert/strict";
import myntraExtractor, {
  matchesMyntra,
  parseInr,
  parseMyntraDate,
  extractOrderId,
  ...
  extractDate,
  extract,
} from "../../src/extractors/myntra.js";
import { runExtractor } from "../../src/extractor.js";

// ── tiny DOM shim (mirrors flipkart test) ────────────────────────────────
...
// ── matchesMyntra ────────────────────────────────────────────────────────
assert.equal(matchesMyntra("https://www.myntra.com/my/orders"), true);
assert.equal(matchesMyntra("https://www.myntra.com/my/orders/1126871234567890-1234567"), true);
assert.equal(matchesMyntra("https://www.myntra.com/my/orders/details/1126871234567890-1234567"), true);
assert.equal(matchesMyntra("https://www.myntra.com/shop/men"), false);
assert.equal(matchesMyntra("https://www.flipkart.com/account/orders"), false);
assert.equal(matchesMyntra(null), false);
assert.equal(matchesMyntra("not-a-url"), false);
...
// ── parseMyntraDate ──────────────────────────────────────────────────────
assert.ok(parseMyntraDate("23 May 2026").startsWith("2026-05-23"));
assert.ok(parseMyntraDate("Ordered on 1 January 2026").startsWith("2026-01-01"));
assert.ok(parseMyntraDate("Delivered on 23rd May 2026").startsWith("2026-05-23"));
assert.ok(parseMyntraDate("23rd May'26").startsWith("2026-05-23"));
assert.ok(parseMyntraDate("2026-05-23").startsWith("2026-05-23"));
assert.equal(parseMyntraDate("not a date"), null);
assert.equal(parseMyntraDate(""), null);

// ── extractOrderId from URL path ─────────────────────────────────────────
assert.equal(
  extractOrderId({ querySelector: () => null }, "https://www.myntra.com/my/orders/1126871234567890-1234567"),
  "1126871234567890-1234567",
);
assert.equal(
  extractOrderId({ querySelector: () => null }, "https://www.myntra.com/my/orders/details/1126871234567890-1234567"),
  "1126871234567890-1234567",
);
// from DOM text when missing from URL
{
  const doc = {
    querySelector: (sel) => sel === "body"
      ? { textContent: "Order ID: 1126877654321098-9876543 placed" }
      : null,
  };
  assert.equal(extractOrderId(doc, "https://www.myntra.com/my/orders"), "1126877654321098-9876543");
}
...
// ── grand-total fallback via summary rows ────────────────────────────────
{
  const root = el("div", { children: [
    ...
    el("li", { attrs: { class: "summary-row" }, text: "Order Total ₹1,250.00" }),
  ]});
...
// ── no total → extractor returns null → runExtractor reports failure ─────
{
  ...
  assert.equal(extract(root, { url: "https://www.myntra.com/my/orders" }), null);
  const result = runExtractor(myntraExtractor, root, { url: "https://www.myntra.com/my/orders" });
  assert.equal(result.ok, false);
  assert.equal(result.error.code, "EMPTY_RESULT");
}

// ── matches() rejects non-myntra URLs through runExtractor ───────────────
{
  const root = el("div");
  const result = runExtractor(myntraExtractor, root, { url: "https://www.flipkart.com/account/orders" });
  assert.equal(result.ok, false);
  assert.equal(result.error.code, "NO_MATCH");
}

// ── extractor object shape ───────────────────────────────────────────────
assert.equal(myntraExtractor.id, "myntra");
assert.equal(typeof myntraExtractor.version, "string");
assert.equal(typeof myntraExtractor.extract, "function");
assert.equal(typeof myntraExtractor.matches, "function");

console.log("\u2713 myntra extractor");
